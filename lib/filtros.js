import { limitesSalario } from "./periodo";
...
export function inicioPeriodo(tipo, customDe, periodicidade) {
  if (tipo === "custom" && customDe) return new Date(`${customDe}T00:00:00`);
  if (tipo === "salario" || tipo === "perfil") return limitesSalario(periodicidade).inicio;
  if (tipo === "semana") {
    const d = new Date();
  ...
  return limitesSalario("mensal").inicio;
}

export function fimPeriodo(tipo, customAte, periodicidade) {
  if (tipo === "custom" && customAte) return new Date(`${customAte}T23:59:59`);
  if (tipo === "salario" || tipo === "perfil") return limitesSalario(periodicidade).fim;
  return new Date();
}

export function filtrarBilhetes(tickets, filtros) {
  const de = inicioPeriodo(filtros.periodo, filtros.de, filtros.periodicidade);
  const ate = fimPeriodo(filtros.periodo, filtros.ate, filtros.periodicidade);
