...
function parseFormatoEsporte(raw) {
  ...
  const manchete = (linhas.find((l) => /^ANALISE ESPORTIVA DO DIA/i.test(l.trim())) || "Boletim do Dia").trim();

  const blocos = texto.split(/^ESPORTE:\s*/im).slice(1);
  const jogos = [];
  ...
  function esporteCanonico(valor) {
    ...
    if (n.includes("tenis") || n.includes("tênis")) return "Tênis";
    if (n.includes("futebol")) return "Futebol";
    return valor.trim() || "Outros";
  }

  function horaDeFormato(valor) {
    const m = String(valor || "").match(/(\d{1,2}h\d{0,2})/i);
    return m ? m[1] : "";
  }

  function adicionar(esporteOriginal, jogo, liga, hora, local, detalhe) {
    if (!jogo) return;
    const esporte = esporteCanonico(esporteOriginal);
    const item = {
      jogo: limpar(jogo),
      principal: false,
      esporte,
      liga: limpar(liga),
      hora: horaDeFormato(hora),
      noticia: limpar(jogo) + (hora ? " — " + horaDeFormato(hora) : "") + (liga ? " — " + limpar(liga) : ""),
      ...
      leitura: "",
      entradas: [],
      extra: local ? ["Local: " + limpar(local)] : [],
    };
    const chave = chaveJogo(item.jogo);
    if (!jogos.some((j) => chaveJogo(j.jogo) === chave)) jogos.push(item);
  }

  for (const bloco of blocos) {
    const linhasBloco = bloco.split("\n");
    const esporteOriginal = (linhasBloco[0] || "Outros").trim();
    const resto = linhasBloco.slice(1).join("\n");
    const liga = campo(resto, "COMPETICAO");
    const dataHora = campo(resto, "DATA/HORA");
    const local = campo(resto, "LOCAL");
    const evento = campo(resto, "EVENTO");
    const eventos = campo(resto, "EVENTOS");
    const partida = campo(resto, "PARTIDA");
    const nome = evento || eventos || partida;

    if (nome) {
      const multiplos = nome.split("|").map((x) => x.trim()).filter(Boolean);
      for (const n of multiplos) adicionar(esporteOriginal, n, liga, dataHora, local, resto);
    }

    const jogosLinha = resto.match(/^JOGOS(?:-CHAVE)?\s*:\s*(.+)$/im)?.[1] || "";
    if (jogosLinha) {
      for (const n of jogosLinha.split("|").map((x) => x.trim()).filter(Boolean)) {
        const h = n.match(/(\d{1,2}h\d{2})/i)?.[1] || dataHora;
        adicionar(esporteOriginal, n.replace(/\s+\d{1,2}h\d{2}.*$/i, "").trim(), liga, h, local, resto);
      }
    }

    const destaques = resto.match(/^DESTAQUES\s*:\s*(.+)$/im)?.[1] || "";
    if (destaques) {
      for (const n of destaques.split("|").map((x) => x.trim()).filter(Boolean)) {
        adicionar(esporteOriginal, n, liga, dataHora, local, resto);
      }
    }
  }

  // Curadoria inicial: futebol recebe os principais confrontos do TXT;
  // os demais esportes entram com pelo menos um destaque.
  const por = (nome) => jogos.filter((j) => j.esporte === nome);
  const marcar = (nome, quantidade) => por(nome).slice(0, quantidade).forEach((j) => { j.principal = true; });

  marcar("Futebol", Math.min(8, por("Futebol").length));
  for (const nome of ["UFC","NASCAR","MLB","NCAA","Golfe","AFL","WNBA","NHL","MotoGP","Tênis"]) marcar(nome, 1);
  for (const j of por("NFL")) j.principal = false;

  const esportes = [];
  for (const j of jogos) if (!esportes.includes(j.esporte)) esportes.push(j.esporte);

  return {
    bruto: raw,
    manchete,
    geral: "Os principais eventos de hoje, reunidos por esporte. Use os filtros para navegar pela edição.",
    jogos,
    esportes,
  };
}
