...
function parseFormatoEsporte(raw) {
  ...
  function extrairEntradas(bloco) {
    ...
    return linhas
      ...
      .filter(Boolean)
      .map((textoEntrada) => {
        const probMatch = textoEntrada.match(/(?:—|-)\s*~?\s*(\d+(?:[.,]\d+)?(?:\s*-\s*\d+(?:[.,]\d+)?)?)\s*%\s*(?:Green)?/i);
        const probabilidade = probMatch?.[1]?.replace(/\s+/g, "") || "";
        const mercado = probMatch
          ? textoEntrada.slice(0, probMatch.index).trim()
          : textoEntrada;
        return {
          mercado,
          odd: "",
  ...
  function adicionarItem({
    ...
    const item = {
      ...
      odd: entradas?.[0]?.odd || "",
      probabilidade: entradas?.[0]?.probabilidade || "",
  ...
  for (const bloco of separarBlocos()) {
    ...
    const estatisticas = campoTexto(bloco, "ESTATISTICAS E PROBABILIDADES");
