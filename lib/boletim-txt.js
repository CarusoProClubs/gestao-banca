...
export function parseBoletimTxt(raw) {
  ...
  // Formato enxuto: ESPORTE + uma linha por jogo usando |.
  // Ex.: Flamengo x Palmeiras | 19h | Brasileirão | Flamengo vence | 2.05 | 54%
  const compacto = [];
  if (!/PARTE\s*1/i.test(texto) && !/PARTE\s*2/i.test(texto)) {
    ...
    for (const linha of linhas.slice(1)) {
      ...
      if (secao) { esporteCompacto = secao[1]; continue; }
      const partes = t.split("|").map((x) => x.trim());
      if (partes.length >= 5) {
        const [jogo, hora, liga, mercado, odd, probabilidade] = partes;
        compacto.push({ jogo, principal: true, esporte: esporteCompacto, liga, hora, noticia: "", detalhe: "", transmissao: "", melhorOdd: "", risco: "", resumo: "", oQuePesa: "", leitura: "", mercado, odd: odd.replace(",", "."), probabilidade, entradas: [{ mercado, odd: odd.replace(",", "."), probabilidade }], extra: [] });
      }
    }
  }
  ...
  if (compacto.length) {
    geral = linhas.slice(1).find((l) => l.trim() && !l.includes("|") && !SECOES.some(([regex]) => regex.test(semAcentos(l.trim()))))?.trim() || "";
  }
