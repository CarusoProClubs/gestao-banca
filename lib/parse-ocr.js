...
function detectStatus(text) {
  ...
  if (/\bperdida\b/.test(n) || /\bberdida\b/.test(n) || /\bbendida\b/.test(n)) return "perdida";
...
function isNoiseLine(line) {
  const n = normalize(line).toLowerCase();
  return /^(simples|dupla|tripla|multipla|ganhos|id:|betano)\b/.test(n) || n.includes("criar aposta") || n.startsWith("id:") || n.startsWith("ganhos") || /\b(perdida|ganhou|berdida|bendida|pendente)\b/.test(n);
}
function cleanLabel(value) {
  return normalize(value).replace(/\b(perdida|ganhou|berdida|bendida|pendente)\b/gi, "").replace(/[\u2122\u00ae]+/g, "").replace(/\(\s*\)/g, "").replace(/^[^A-Za-z0-9]+/, "").replace(/\s+/g, " ").trim();
}
function cleanJogo(value) {
  if (!value) return null;
  const cleaned = normalize(value).replace(/\s*\(\s*\d*$/g, "").replace(/\s*\(\s*$/g, "").replace(/\s*Pontuac.*$/i, "").replace(/\s+/g, " ").trim();
...
export function parseOcrText(raw) {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (/\([^)]+\)/.test(line) && !/CA\s/i.test(line) && !/ID:/.test(line) && !/\b(perdida|ganhou|berdida|bendida)\b/i.test(line)) {
  ...
  }).map((perna, index) => ({ ...perna, ordem: index + 1 }));
  ...
  for (const perna of pernasLimpas) {
    perna.jogo = cleanJogo(perna.jogo);
