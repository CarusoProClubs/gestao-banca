...
function detectCasa(text) {
  const n = normalize(text).toLowerCase();
  const casas = [
    ["Betano", /\bbetano\b/],
    ["bet365", /\bbet365\b/],
    ["Betfair", /\bbetfair\b/],
    ["Sportingbet", /\bsportingbet\b/],
    ["KTO", /\bkto\b/],
    ["Novibet", /\bnovibet\b/],
    ["EstrelaBet", /\bestrelabet\b/],
    ["Superbet", /\bsuperbet\b/],
    ["Blaze", /\bblaze\b/],
    ["Pixbet", /\bpixbet\b/],
    ["Stake", /\bstake\b/],
    ["1xBet", /\b1xbet\b/],
  ...
  return casas.find(([, pattern]) => pattern.test(n))?.[0] ?? null;
}
...
function isNoiseLine(line) {
  const n = normalize(line).toLowerCase();
  return /^(simples|dupla|tripla|multipla|ganhos|id:|betano|bet365|betfair|sportingbet|kto|novibet|estrelabet|superbet|blaze|pixbet|stake|1xbet|rivalo)\b/.test(n) || n.includes("criar aposta") || n.startsWith("id:") || n.startsWith("ganhos") || /\b(perdida|ganhou|berdida|bendida|berdidg|pendente)\b/.test(n);
}
...
export function parseOcrText(raw) {
  ...
  const avisos = ["Leitura automática do print. Confira os campos antes de confirmar."];
  const casa = detectCasa(blob);
  if (!casa) avisos.push("Casa de aposta não identificada automaticamente. Selecione a casa correta.");
  ...
  const bookingMatch = blob.match(/(?:booking|c[oó]digo(?:\s+da)?\s+aposta)\s*[:#-]?\s*([A-Z0-9-]{5,})/i);
  ...
  const pushPerna = (selecao, extra = {}) => {
    ...
    pernas.push({ ordem: pernas.length + 1, jogo: extra.jogo ?? null, selecao: clean, mercado: extra.mercado ?? null, odd_perna: extra.odd_perna ?? null, placar_print: extra.placar_print ?? null, status_print: extra.status_print ?? "desconhecido", status_usuario: "pendente" });
  };
  ...
  return {
    ...
    codigo_booking: bookingMatch?.[1] ?? null,
    ...
    pernas: pernasLimpas,
    confianca: casa && idMatch && valor ? 0.85 : casa || idMatch || valor ? 0.65 : 0.4,
    avisos,
  };
}
