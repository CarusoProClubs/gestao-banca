...
export function fecharBilhete(bilhete) {
  ...
  // Somente pendente pode ser derivado do estado das pernas.
  // Uma decisão explícita do usuário (green/red/anulada/cashout)
  // nunca é sobrescrita por inferência automática.
  if (status_usuario === "pendente") {
    // Resultado financeiro só pode ser derivado de decisões manuais nas pernas.
    // status_print é evidência do print/IA, não autorização para fechar o bilhete.
    const statusesManuais = normalizedPernas.map((perna) => perna?.status_usuario || "pendente");
    if (statusesManuais.some((status) => status === "red")) status_usuario = "red";
    else if (normalizedPernas.length && statusesManuais.every((status) => status === "green" || status === "anulada")) {
  ...
  ticket.avisos = [...new Set(ticket.avisos)];
