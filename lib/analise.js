...
export function resumirAnalise(tickets) {
  ...
  return {
    ...
    porMercado: agrupar(tickets, (t) => {
      const pernas = t.payload?.pernas || [];
      return pernas[0]?.mercado || t.formato || t.tipo || "outros";
    }),
    porFaixaOdd: agrupar(tickets, (t) => faixaOdd(t.odd_bilhete)),
    porTipo: agrupar(tickets, (t) => [t.tipo, t.formato].filter(Boolean).join(" / ") || "—"),
    porCasa: agrupar(tickets, (t) => t.casa),
  };
}
