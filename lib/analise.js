...
import { entidadesDoBilhete } from "./entidades";
...
function agrupar(tickets, chaveFn) {
  ...
  for (const ticket of tickets) {
    const keys = chaveFn(ticket);
    const lista = Array.isArray(keys) ? keys : [keys];
    for (const key of lista.filter(Boolean)) {
      if (!map.has(key)) {
        map.set(key, {
          nome: key,
          qtd: 0,
      ...
      if (ticket.status_usuario === "cashout") row.cashout += 1;
      if (ticket.status_usuario === "anulada") row.anulada += 1;
      if (ticket.status_usuario === "pendente") row.pendente += 1;
  ...
  return [...map.values()]
    .map((row) => ({
      ...row,
      fechados: row.green + row.red,
      acerto: row.green + row.red > 0 ? row.green / (row.green + row.red) : 0,
      roi: row.apostado > 0 ? row.lucro / row.apostado : 0,
    }))
    .sort((a, b) => b.lucro - a.lucro);
}

function mercadoDoBilhete(ticket) {
  const pernas = ticket.payload?.pernas || ticket.pernas || [];
...
export function resumirAnalise(tickets) {
  const fechados = tickets.filter((t) => ["green", "red"].includes(t.status_usuario));
  ...
  return {
    ...
    porLiga: agrupar(tickets, (t) => entidadesDoBilhete(t).ligas),
    porClube: agrupar(tickets, (t) => entidadesDoBilhete(t).clubes),
    porJogador: agrupar(tickets, (t) => entidadesDoBilhete(t).jogadores),
  };
}
